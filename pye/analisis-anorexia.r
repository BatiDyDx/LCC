...
pie(table.signo, main = c("PRINCIPAL SIGNO VISIBLE EN PACIENTES CON ANOREXIA",
                           "ARGENTINA, OCTUBRE 2012"),
    labels = labels, clockwise = TRUE)

# c)
percents <- round(table.signo / length(Signo), digits = 4) * 100
...
pie(table.signo, main = title, labels = labels, clockwise = TRUE)

# d)
text = "Fuente: Asosiacion de Lucha contra la Bulimia y Anorexia"
mtext(text = text, side = 1, line = 2, at = 0, adj = 0, cex = 0.8)
...
mtext(text, side = 1, line = 4, cex = 0.8, at = 0, adj = 0)
...
mtext(text, side = 1, line = 4, at = 0, adj = 0)
...
plot(x = 0:6, y = data, type = "h", ylim = c(0,25),
     xlab = "Numero de visitas", ylab = "Frecuencia absoluta",
     main = main.title)

mtext(text, side = 1, line = 4, at = 0.75, adj = 0)

# c)
prop.ac <- c(0, round(cumsum(freq) / length(Num.Visitas), digits = 4), 1)
...
mtext(text, side = 1, line = 4, at = 1, adj = 0)
